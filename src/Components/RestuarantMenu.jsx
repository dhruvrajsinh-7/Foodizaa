...
import { ShimmerMenu } from "./pages/Shimmer";
...
const RestaurantMenu = () => {
  ...
  return !menu ? (
    ...
    <div className="flex flex-col w-[80%] md:w-2/3 p-4 border m-auto">
      <div className="flex flex-col rounded-md justify-between pb-4 border-b md:flex-row gap-3">
        <div className="flex flex-col text-sm text-[#535665] font-medium gap-1">
          ...
          <span className="">
            {menu?.cards[0]?.card?.card?.info?.areaName},{" "}
            {menu?.cards[0]?.card?.card?.info?.city}{" "}
            <i className="text-blue-600 fas fa-location-dot"></i>
          </span>
          <span className="">
            {menu?.cards[0]?.card?.card?.info?.avgRating}{" "}
            <i className="text-green-600 fas fa-star"></i> |{" "}
            {menu?.cards[0]?.card?.card?.info?.totalRatingsString}
          </span>
          <span className="">
            {menu?.cards[0]?.card?.card?.info?.feeDetails?.message}
          </span>
        </div>
        <img
          className="w-56 h-36 rounded-lg"
          src={IMG_CDN + menu?.cards[0]?.card?.card?.info?.cloudinaryImageId}
          alt=""
        />
      </div>
      <div className="flex gap-8 items-center border-b py-3 text-sm md:text-base">
        <div className="flex items-center gap-2 font-semibold">
          ...
          <span className="">
            {menu?.cards[0]?.card?.card?.info?.sla?.slaString}
          </span>
        </div>
        <div className="flex items-center gap-2 font-semibold">
          <svg
      ...
      <h1 className="font-bold mt-4 mb-10">
        Recommended (
        {
          menu?.cards[menu?.cards.length - 1]?.groupedCard?.cardGroupMap
                ...
                <div className="flex flex-col gap-2 w-full md:w-3/4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {item?.card?.info?.offerTags && (
                      <span className="text-xs font-medium px-[6px] bg-red-100 text-orange-500">
